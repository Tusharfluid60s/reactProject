...
const Videos = () => {
  ...
  const [videoSrc, setVideoSrcc] = useState(videosArr[0]);
    ...
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflow={'auto'}>
          ...
          <Text>
          {/* overflow auto will give a scroll mode */}
            This is a sample video for testing and demo. This is called
            description.lllllllllllllllllllllllllllllllllllllll
            l
      ...
      <VStack
        ...
        overflowY={'auto'}
      >
        {videosArr.map((i, index) => (
          <Button
            variant={'ghost'}
            colorScheme={'messenger'}
            onClick={() => setVideoSrcc(i)}
          >
            Lecture {index + 1}
          </Button>
        ))}
